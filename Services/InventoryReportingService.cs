...
public record DoctorUsage(Guid DentistUserId, string DentistName, decimal UsageQuantity, decimal WasteQuantity, decimal EstimatedCost);
...
public record UsageCostReport(
    ...
    IReadOnlyList<DoctorUsage> UsageByDoctor);
...
public class InventoryReportingService(ZeppelinDbContext db, InventoryService inventoryService)
{
    ...
    public async Task<UsageCostReport> GetUsageCostReportAsync(DateOnly from, DateOnly to, InventoryCategory? category)
    {
        ...
        var movementsQuery = db.StockMovements
            ...
            .Include(m => m.Appointment!).ThenInclude(a => a.DentistUser)
        ...
        // Only movements logged against a specific appointment carry a dentist -
        // generic/manual adjustments from the Inventory page have no patient
        // attribution and are excluded here rather than lumped under "unknown".
        var usageByDoctor = movements
            .Where(m => m.Appointment is not null)
            .GroupBy(m => m.Appointment!.DentistUserId)
            .Select(g => new DoctorUsage(
                g.Key,
                g.First().Appointment!.DentistUser is { } d ? $"{d.FirstName} {d.LastName}" : "Unknown",
                g.Where(m => m.Type == StockMovementType.UsageDeduction).Sum(m => m.Quantity),
                g.Where(m => m.Type == StockMovementType.Waste).Sum(m => m.Quantity),
                g.Sum(CostOf)))
            .OrderByDescending(d => d.EstimatedCost)
            .ToList();
